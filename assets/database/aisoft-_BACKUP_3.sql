...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 10-07-2024 a las 11:38:02
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_add_quota` (IN `_idcontrato` INT, IN `_nro_cuota` INT, IN `_monto_cuota` DECIMAL(8,2), IN `_fecha_vencimiento` DATE, IN `_idusuario` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_add_user` (IN `_imagen` VARCHAR(100), IN `_nombres` VARCHAR(40), IN `_apellidos` VARCHAR(40), IN `_documento_tipo` VARCHAR(30), IN `_documento_nro` VARCHAR(12), IN `_estado_civil` VARCHAR(10), IN `_iddistrito` INT, IN `_direccion` VARCHAR(60), IN `_nacionalidad` VARCHAR(20), IN `_correo` VARCHAR(60), IN `_contrasenia` VARCHAR(60), IN `_idrol` INT, IN `_idsede` INT)   BEGIN
    DECLARE _existUser INT;
    DECLARE _idpersonAdd  INT;
    SET _existUser = (
        SELECT idpersona FROM personas
        WHERE inactive_at IS NULL
        AND documento_nro = _documento_nro
    );
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_calculate_clients` (IN `_idproyecto` INT)   BEGIN
    SELECT DISTINCT
            "CONTRATO" AS tipo,
            cn.idcontrato,
            cn.idactivo,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_calculate_debt` (IN `_idcontrato` INT)   BEGIN
   -- Variables de sesión
SET @calculate := 0;
SET @monto := 0;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_get_last_sales` (IN `_idusuario` INT)   BEGIN
    SELECT 
        cn.idcontrato,
        cn.n_expediente,
        cn.denominacion,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_get_user` (IN `_idusuario` INT)   BEGIN
    SELECT 
            us.idusuario,
            us.imagen,
            us.correo,
            ...
            ps.nacionalidad,
            sb.sede,
            sb.idsede AS idsede_sede,
            sb.iddepartamento AS iddepartamento_sede,
            sb.idprovincia AS idprovincia_sede,
            sb.iddistrito AS iddistrito_sede
        FROM usuarios as us
        ...
        )AS sb ON sb.idsede = us.idsede
        WHERE us.idusuario = _idusuario;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_grafic_quotas` ()   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_grafic_refunds_money` ()   BEGIN
	SELECT 
		moneda_venta,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_grafic_sellers` ()   BEGIN
	SELECT 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_inactive_user` (IN `_idusuario` INT)   BEGIN
    UPDATE usuarios        
        SET 
            inactive_at = CURDATE()
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_list_rols` ()   BEGIN
    SELECT * FROM roles;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_list_users` ()   BEGIN
    SELECT 
        us.idusuario,
        CONCAT(UPPER(ps.apellidos),', ',LOWER(ps.nombres)) AS usuario,
        us.correo,
        ps.documento_tipo,
        ps.documento_nro,
        rl.rol
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_reports_cuotas` (IN `_idcontrato` INT)   BEGIN
    SELECT ct.idcuota,
            ct.nro_cuota,
            ct.idcontrato,
            ct.monto_cuota,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_reports_cuotas_extend` (IN `_idcontrato` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_reports_det_budgets` (IN `_idactivo` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_reports_projects` ()   BEGIN
    SELECT 
        py.idproyecto,
        py.codigo,
        py.denominacion,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_set_user` (IN `_idusuario` INT, IN `_imagen` VARCHAR(100), IN `_idpersona` INT, IN `_nombres` VARCHAR(40), IN `_apellidos` VARCHAR(40), IN `_documento_tipo` VARCHAR(30), IN `_documento_nro` VARCHAR(12), IN `_estado_civil` VARCHAR(10), IN `_iddistrito` INT, IN `_direccion` VARCHAR(60), IN `_nacionalidad` VARCHAR(20), IN `_correo` VARCHAR(60), IN `_idrol` INT, IN `_idsede` INT)   BEGIN
    ...
    UPDATE usuarios 
        ...
        SET _count2 = (SELECT ROW_COUNT());

    IF _count1 = 0 THEN
        SELECT _count1 AS filasAfect;
    ELSE
        SELECT _count2 AS filasAfect;
    END IF;
...
(1, 5, 'LOTE', 'cac10d67f7877a2085c6625203e7f62c88538d2ajpg', 'VENDIDO', 1, 'av centenario #104', 'SOL', 50.00, 0.00, 0.00, 50, 'partida electronica nro 4', NULL, NULL, '{\"clave\":[\"\"],\"valor\":[\"\"]}', '{\"clave\" :[], \"valor\":[]}', NULL, 'TERCEROS', 20000.00, NULL, 20000.00, '2024-06-18', '2024-07-09', NULL, 3),
...
(11, 3, 'CASA', 'f033614371185e66c15df0b2fa8d6dec5bfe403bjpg', 'SIN VENDER', 2, '123', 'SOL', 14.00, NULL, 12.00, 14, '56', NULL, NULL, '{\"clave\":[\"\"],\"valor\":[\"\"]}', '{\"clave\" :[], \"valor\":[]}', NULL, 'A.I.F', 40000.00, NULL, 40000.00, '2024-06-29', '2024-07-09', NULL, 3),
(12, 1, 'CASA', 'c070115c4604186db530726f227724454afbf676jpg', 'SIN VENDER', 1, '23', 'SOL', 12.00, NULL, 50.00, 12, '123', NULL, NULL, '{\"clave\":[\"\"],\"valor\":[\"\"]}', '{\"clave\" :[], \"valor\":[]}', NULL, 'A.I.F', 60000.00, NULL, 60000.00, '2024-06-29', '2024-07-09', NULL, 3),
(13, 1, 'CASA', 'e7a9045d7825b550da0f70fdf90dad4c83e95092jpg', 'SIN VENDER', 2, '123', 'SOL', 13.00, NULL, 50.00, 13, '123', NULL, NULL, '{\"clave\":[\"\"],\"valor\":[\"\"]}', '{\"clave\" :[], \"valor\":[]}', NULL, 'A.I.F', 60000.00, NULL, 60000.00, '2024-06-29', '2024-07-09', NULL, 3),
(14, 2, 'CASA', '732453d72ec3037a5da93b2a3012a32e43752be9jpg', 'SIN VENDER', 1, '123', 'SOL', 12.00, NULL, 12.00, 12, '123', NULL, NULL, '{\"clave\":[\"\"],\"valor\":[\"\"]}', '{\"clave\" :[], \"valor\":[]}', NULL, 'A.I.F', 40000.00, NULL, 40000.00, '2024-06-29', '2024-07-09', NULL, 3),
(15, 2, 'CASA', 'f904a25f250ee58203273b98c0a6d21417fa46a6jpg', 'SIN VENDER', 2, '123', 'SOL', 12.00, NULL, 12.00, 12, '123', NULL, NULL, '{\"clave\":[\"\"],\"valor\":[\"\"]}', '{\"clave\" :[], \"valor\":[]}', NULL, 'A.I.F', 45000.00, NULL, 45000.00, '2024-06-29', '2024-07-09', NULL, 3),
(16, 4, 'LOTE', '10dd0488ef56584f73be4be6e3f9d4dc920e1a02jpg', 'VENDIDO', 4, 'dirccion', 'USD', 12.00, NULL, NULL, 12, '123', NULL, NULL, '{\"clave\":[\"\"],\"valor\":[\"\"]}', '{\"clave\" :[], \"valor\":[]}', NULL, 'A.I.F', 5000.00, NULL, 5000.00, '2024-07-04', '2024-07-04', NULL, 1),
(17, 5, 'CASA', '24483ce92236cbe04812e57ac5325455bd06b26d.jpg', 'SIN VENDER', 9, 'av centenario #103', 'USD', 50.00, 40.00, 50.00, 10, 'nro 1', NULL, NULL, '{\"clave\":[\"\"],\"valor\":[\"\"]}', '{\"clave\" :[], \"valor\":[]}', 12, 'A.I.F', 5000.00, 14830.40, 19830.40, '2024-07-08', '2024-07-09', NULL, 3),
...
(7, 'NATURAL', 20, NULL, '2024-07-09', NULL, NULL, 3),
(8, 'JURÍDICA', NULL, 6, '2024-07-09', NULL, NULL, 1);
...
INSERT INTO `contratos` (`idcontrato`, `n_expediente`, `tipo_contrato`, `idseparacion`, `idrepresentante_primario`, `idrepresentante_secundario`, `idcliente`, `idconyugue`, `idactivo`, `tipo_cambio`, `estado`, `fecha_contrato`, `precio_venta`, `moneda_venta`, `inicial`, `det_contrato`, `archivo`, `create_at`, `update_at`, `inactive_at`, `idusuario`, `idvendedor`) VALUES
(1, 'CONT-00004', 'VENTA DE LOTE', 1, 1, NULL, 1, NULL, NULL, 3.870, 'INACTIVO', '2024-06-27', 20000.00, 'SOL', 500.00, '{\"clave\":[\"1\"],\"valor\":[\"1\"]}', '9590e2cb47dcd69d6d8cf1f80df2899714b1535f.pdf', '2024-06-27', NULL, '2024-07-03', 1, 7),
(2, 'CONT-00005', 'VENTA DE LOTE', 2, 1, NULL, 1, NULL, NULL, 3.800, 'VIGENTE', '2024-06-28', 25000.00, 'SOL', 500.00, '{\"clave\":[\"1\"],\"valor\":[\"2\"]}', '00a925ae7a499407ce38a08e8fb00dbec84f3ee9.pdf', '2024-06-28', NULL, NULL, 1, 5),
(3, 'CONT-00006', 'VENTA DE CASA', NULL, 1, NULL, 1, NULL, 3, 3.500, 'VIGENTE', '2024-06-28', 50000.00, 'SOL', 500.00, '{\"clave\":[\"1\"],\"valor\":[\"1\"]}', '4aae50989f4a013eb07e774bb3665ec74015d86f.pdf', '2024-06-28', '2024-07-06', NULL, 3, 13),
(4, 'CONT-00007', 'VENTA DE CASA', NULL, 1, NULL, 1, NULL, 4, 3.900, 'INACTIVO', '2024-06-28', 50700.00, 'SOL', 600.00, '{\"clave\":[\"1\"],\"valor\":[\"1\"]}', '4ffd99ced216ab0e5faf44b57f2e35757a9192ec.pdf', '2024-06-28', NULL, '2024-07-03', 1, 5),
(5, 'CONT-00008', 'VENTA DE LOTE', 3, 1, NULL, 6, NULL, NULL, 3.830, 'VIGENTE', '2024-06-28', 5000.00, 'USD', 590.00, '{\"clave\":[\"fecha de vencimiento\",\"tramites legales\",\"cmpromiso de pago\"],\"valor\":[\"el contrato vende dentro de 3 años\",\"se hicieron varios tramites\",\"el cliente se compromete a pagar\"]}', 'f177fa693ccfcbe7d206af5bac84ca3079aeb795.pdf', '2024-06-28', '2024-06-29', NULL, 1, 7),
(6, 'CONT-00009', 'VENTA DE CASA', NULL, 1, NULL, 1, NULL, 5, 3.500, 'INACTIVO', '2024-06-28', 50464.40, 'SOL', 600.00, '{\"clave\":[\"1\"],\"valor\":[\"1\"]}', 'df0065c11ffa9ab5c58581f941cb3e473e3f6b96.pdf', '2024-06-28', NULL, '2024-06-29', 1, 14),
(7, 'CONT-00010', 'VENTA DE LOTE', 1, 1, NULL, 1, NULL, NULL, 3.830, 'VIGENTE', '2024-07-03', 20000.00, 'SOL', 500.00, '{\"clave\":[\"12\"],\"valor\":[\"21\"]}', '994e3215c9d75ba90d8410b2b0b196ff9a5d765c.pdf', '2024-07-03', NULL, NULL, 1, 12),
...
(55, 9, 1000.00, '2024-11-05', 'POR CANCELAR', '2024-07-06', NULL, NULL, 3, 4),
(56, 9, 1000.00, '2024-12-05', 'POR CANCELAR', '2024-07-06', NULL, NULL, 3, 5),
(57, 9, 1000.00, '2025-01-05', 'POR CANCELAR', '2024-07-06', NULL, NULL, 3, 6),
(58, 9, 1000.00, '2025-02-05', 'POR CANCELAR', '2024-07-06', NULL, NULL, 3, 7),
(59, 9, 1000.00, '2025-03-05', 'POR CANCELAR', '2024-07-06', NULL, NULL, 3, 8),
(60, 9, 1000.00, '2025-04-05', 'POR CANCELAR', '2024-07-06', NULL, NULL, 3, 9),
(61, 9, 1000.00, '2025-05-05', 'POR CANCELAR', '2024-07-06', NULL, NULL, 3, 10),
(62, 9, 1000.00, '2025-06-05', 'POR CANCELAR', '2024-07-06', NULL, NULL, 3, 11),
(63, 9, 1000.00, '2025-07-05', 'POR CANCELAR', '2024-07-06', NULL, NULL, 3, 12),
(64, 9, 1000.00, '2025-08-05', 'POR CANCELAR', '2024-07-06', NULL, NULL, 3, 13),
(65, 9, 1000.00, '2025-09-05', 'POR CANCELAR', '2024-07-06', NULL, NULL, 3, 14),
(66, 9, 1000.00, '2025-10-05', 'POR CANCELAR', '2024-07-06', NULL, NULL, 3, 15),
(67, 9, 1000.00, '2025-11-05', 'POR CANCELAR', '2024-07-06', NULL, NULL, 3, 16),
(68, 9, 1000.00, '2025-12-05', 'POR CANCELAR', '2024-07-06', NULL, NULL, 3, 17),
(69, 9, 1000.00, '2026-01-05', 'POR CANCELAR', '2024-07-06', NULL, NULL, 3, 18),
(70, 9, 1000.00, '2026-02-05', 'POR CANCELAR', '2024-07-06', NULL, NULL, 3, 19),
(71, 9, 1000.00, '2026-03-05', 'POR CANCELAR', '2024-07-06', NULL, NULL, 3, 20),
(72, 10, 1000.00, '2024-08-07', 'POR CANCELAR', '2024-07-08', '2024-07-08', NULL, 3, 1),
(73, 10, 1000.00, '2024-09-07', 'CANCELADO', '2024-07-08', '2024-07-09', NULL, 1, 2),
(74, 10, 1000.00, '2024-10-07', 'CANCELADO', '2024-07-08', '2024-07-09', NULL, 1, 3),
...
(85, 10, 1000.00, '2025-09-07', 'POR CANCELAR', '2024-07-08', NULL, NULL, 3, 14),
(86, 10, 1000.00, '2025-10-07', 'POR CANCELAR', '2024-07-08', NULL, NULL, 3, 15),
(87, 10, 1000.00, '2025-11-07', 'POR CANCELAR', '2024-07-08', NULL, NULL, 3, 16),
(88, 10, 1000.00, '2025-12-07', 'POR CANCELAR', '2024-07-08', NULL, NULL, 3, 17),
...
(2, 8, 5, '2024-06-29', NULL, NULL),
(3, 8, 5, '2024-06-29', NULL, NULL),
(4, 9, 8, '2024-07-04', NULL, NULL),
(5, 10, 11, '2024-07-09', NULL, NULL);
...
(5, 13, 10, NULL, 'TARRAJEO', 1, 350.00, '2024-06-19', NULL, '2024-07-09', 1),
...
(8, 12, 13, NULL, 'detalle', 1, 500.00, '2024-07-05', NULL, NULL, 3),
(9, 12, 10, NULL, 'FLIT', 1, 22.00, '2024-07-05', NULL, NULL, 3),
(10, 12, 10, NULL, 'INSTALACIONES DE PUERTAS', 4, 250.00, '2024-07-05', NULL, NULL, 3),
(13, 12, 10, NULL, 'BLOQUE DE VENTANAS', 4, 250.00, '2024-07-05', NULL, NULL, 3),
(14, 12, 10, NULL, 'BLOQUE DE PUERTAS', 4, 250.00, '2024-07-05', NULL, NULL, 3),
(15, 12, 1, 3, 'APU // Bolsa de Cemento APU 50kg // kg', 30, 26.00, '2024-07-05', NULL, NULL, 3),
(16, 12, 1, 30, 'APU // Sumidero de 2\" // tonelada métrica', 20, 5.00, '2024-07-05', NULL, NULL, 3),
(17, 12, 12, NULL, 'PAGOS AL BANCO', 1, 2500.00, '2024-07-05', NULL, NULL, 3),
(18, 12, 16, NULL, 'PAGO DE IMPUESTOS', 1, 5000.00, '2024-07-05', NULL, NULL, 3),
(19, 12, 14, NULL, 'PAGO POR EJECUCION', 2, 300.00, '2024-07-05', NULL, NULL, 3),
(20, 12, 1, 13, 'Acindar // Bolsa de Arena fina // metro lineal', 10, 94.40, '2024-07-09', NULL, NULL, 3),
(21, 12, 1, 40, 'Acindar // Revestimiento cerámico 60x60 // tonelada métrica', 6, 70.00, '2024-07-09', NULL, NULL, 3),
(22, 12, 13, NULL, 'habilitacón', 1, 500.00, '2024-07-09', NULL, NULL, 3),
(23, 13, 7, 29, 'Pavco // Registro roscado de 4\" // tonelada métrica', 2, 13.00, '2024-07-09', NULL, NULL, 3),
(24, 13, 7, 1, 'Pavco // Tubo PVC 1\" // unidad', 2, 25.00, '2024-07-09', NULL, NULL, 3),
(25, 13, 7, 2, 'Pavco // Tubo PVC 2\" // unidad', 2, 35.00, '2024-07-09', NULL, NULL, 3),
(26, 13, 1, 3, 'APU // Bolsa de Cemento APU 50kg // kg', 127, 26.00, '2024-07-09', NULL, NULL, 3),
(27, 13, 1, 3, 'APU // Bolsa de Cemento APU 50kg // kg', 54, 25.50, '2024-07-09', NULL, NULL, 3),
(28, 13, 1, 4, 'Majestad // Gallón de Pintura Látex // ml', 1, 65.00, '2024-07-09', NULL, NULL, 3),
(29, 13, 1, 12, 'Trébol // Ladrillo 18 huecos // metro', 127, 1.00, '2024-07-09', NULL, NULL, 3),
(30, 13, 4, NULL, 'arena fina', 7, 94.00, '2024-07-09', NULL, NULL, 3),
(31, 13, 4, NULL, 'arena gruesa', 17, 19.00, '2024-07-09', NULL, NULL, 3),
(32, 13, 15, NULL, 'copia iteral traimite 1', 1, 16.00, '2024-07-09', NULL, NULL, 3),
(33, 13, 15, NULL, 'tramite copia literal 2', 1, 16.00, '2024-07-09', NULL, NULL, 3),
(34, 13, 15, NULL, 'pago de certificado negativo de propiedad', 1, 24.00, '2024-07-09', NULL, NULL, 3),
(35, 14, 1, 3, 'APU // Bolsa de Cemento APU 50kg // kg', 100, 26.00, '2024-07-09', NULL, NULL, 3),
(36, 14, 1, 3, 'APU // Bolsa de Cemento APU 50kg // kg', 10, 26.00, '2024-07-09', NULL, NULL, 3),
(37, 14, 9, 40, 'Acindar // Revestimiento cerámico 60x60 // tonelada métrica', 2, 70.00, '2024-07-09', NULL, NULL, 3),
(38, 14, 15, NULL, 'pago por comision de venta de lote', 1, 1500.00, '2024-07-09', NULL, NULL, 3),
(39, 14, 15, NULL, 'elaboración de trámites notariales 1', 1, 65.00, '2024-07-09', NULL, NULL, 3),
(40, 14, 15, NULL, 'elaboración de tráemites notariales 2', 1, 65.00, '2024-07-09', NULL, NULL, 3),
(41, 14, 8, 18, 'Cifial // Tubo agua 3/4\" // juego', 10, 6.40, '2024-07-09', NULL, NULL, 3),
(42, 14, 8, 1, 'Pavco // Tubo PVC 1\" // unidad', 2, 25.00, '2024-07-09', NULL, NULL, 3),
(43, 14, 8, 42, 'Eternit // Tubo de plástico 4\" // tonelada métrica', 3, 40.50, '2024-07-09', NULL, NULL, 3),
(44, 14, 3, 4, 'Majestad // Gallón de Pintura Látex // ml', 2, 65.00, '2024-07-09', NULL, NULL, 3);
...
(3, 3, 50000.00, '2024-07-28', 'pgago por la casa', 'TRANFERENCIA', 'INTERBANCK', '', '6a87eac11c709755d4ed2b2f9b57931cd8334247.jpg', '2024-06-28', NULL, NULL),
(4, 4, 50700.00, '2024-07-28', 'pago por el contrato', 'TRANFERENCIA', 'BCP', '', '6a87eac11c709755d4ed2b2f9b57931cd8334247.jpg', '2024-06-28', NULL, NULL),
...
(25, 73, 500.00, '2024-07-09', 'pago de la primera cuota 2n segunda parte', 'TRANSFERENCIA', 'BCP', '0000000569', '6d474c45684d9d81fadd0f20518eaef138147989.jpg', '2024-07-09', NULL, NULL),
(26, 74, 1000.00, '2024-07-09', 'pago de la segunda cuota', 'TRANSFERENCIA', 'INTERBANCK', '0000000256', '6d474c45684d9d81fadd0f20518eaef138147989.jpg', '2024-07-09', NULL, NULL),
(27, 75, 500.00, '2024-07-09', 'pago por la tercera cuota (primera parte)', 'EFECTIVO', NULL, NULL, '6d474c45684d9d81fadd0f20518eaef138147989.jpg', '2024-07-09', NULL, NULL),
...
(223, 1, 'assets/list_clients', '2024-07-07', NULL, NULL),
(224, 2, 'assets/list_clients', '2024-07-07', NULL, NULL),
(225, 3, 'assets/list_clients', '2024-07-07', NULL, NULL),
(226, 4, 'assets/list_clients', '2024-07-07', NULL, NULL),
(227, 5, 'assets/list_clients', '2024-07-07', NULL, NULL),
(228, 6, 'assets/list_clients', '2024-07-07', NULL, NULL),
(229, 7, 'assets/list_clients', '2024-07-07', NULL, NULL),
(230, 8, 'assets/list_clients', '2024-07-07', NULL, NULL);
...
(20, 'SAMUEL ELISEO', 'ATUNCAR VALERIO', 'DNI', '77068571', 'soltero', 1016, 'av snta rosa #541', 'peruana', '2024-07-09', NULL, NULL);
...
(6, 'AGROLIGHT PERU S.A.C.', 'RUC', '20552103816', 1317, 'PJ. JORGE BASADRE NRO. 158 URB. POP LA UNIVERSAL 2DA ET., LIMA - LIMA ', '2024-07-09', NULL, NULL);
...
(12, 'PRES-00001', 'casa propia', 40.00, '2024-06-19', '2024-07-05', NULL, 3),
(13, 'PRES-00002', 'casa andina', 50.00, '2024-06-19', NULL, NULL, 1),
(14, 'PRES-00005', 'casa playa', 60.00, '2024-06-29', '2024-07-09', NULL, 3),
...
--
DELIMITER $$
CREATE TRIGGER `trgr_update_idpresupuesto` AFTER UPDATE ON `presupuestos` FOR EACH ROW BEGIN
	DECLARE _idactivo INT;
...
(10, 6, 'RUIZ CARLOS LEONOR YOLANDA', 'DNI', '06099879', 'GERENTE GENERAL', 'partida nro #203', 'HABILITADO', '2024-07-09', NULL, NULL);
...
,`expediente_devuelto` varchar(10)
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vws_list_contracts`  AS SELECT `ct`.`idcontrato` AS `idcontrato`, `ct`.`n_expediente` AS `n_expediente`, `ct`.`idcliente` AS `idcliente`, `ct`.`idconyugue` AS `idconyugue`, `ct`.`tipo_contrato` AS `tipo_contrato`, `cl`.`tipo_persona` AS `tipo_persona`, `ac`.`idactivo` AS `idactivo`, `ac`.`sublote` AS `sublote`, `py`.`idproyecto` AS `idproyecto`, `py`.`denominacion` AS `denominacion`, coalesce(concat(ucase(`ps`.`apellidos`),', ',lcase(`ps`.`nombres`)),`pj`.`razon_social`) AS `cliente`, coalesce(`ps`.`documento_tipo`,`pj`.`documento_tipo`) AS `documento_tipo`, coalesce(`ps`.`documento_nro`,`pj`.`documento_nro`) AS `documento_nro`, concat(ucase(`psc`.`apellidos`),', ',lcase(`psc`.`nombres`)) AS `conyugue`, `psc`.`documento_tipo` AS `documento_tipo_cn`, `psc`.`documento_nro` AS `documento_nro_cn`, `ct`.`precio_venta` AS `precio_venta`, `ct`.`moneda_venta` AS `moneda_venta`, `ct`.`inicial` AS `inicial`, `ct`.`tipo_cambio` AS `tipo_cambio`, `ct`.`fecha_contrato` AS `fecha_contrato`, `ct`.`estado` AS `estado`, `ct`.`inactive_at` AS `inactive_at`, `ct`.`idvendedor` AS `idvendedor` FROM ((((((((`contratos` `ct` left join `separaciones` `sp` on(`sp`.`idseparacion` = `ct`.`idseparacion`)) left join `clientes` `cl` on(`ct`.`idseparacion` is null and `cl`.`idcliente` = `ct`.`idcliente` or `ct`.`idseparacion` is not null and `cl`.`idcliente` = `sp`.`idcliente`)) left join `personas_juridicas` `pj` on(`pj`.`idpersona_juridica` = `cl`.`idpersona_juridica`)) left join `personas` `ps` on(`ps`.`idpersona` = `cl`.`idpersona`)) left join `clientes` `cn` on(`cn`.`idcliente` = `ct`.`idconyugue`)) left join `personas` `psc` on(`psc`.`idpersona` = `cn`.`idpersona`)) left join `activos` `ac` on(`ct`.`idactivo` is null and `ac`.`idactivo` = `sp`.`idactivo` or `ct`.`idactivo` is not null and `ac`.`idactivo` = `ct`.`idactivo`)) join `proyectos` `py` on(`py`.`idproyecto` = `ac`.`idproyecto`)) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vws_list_quotas`  AS SELECT `qt`.`nro_cuota` AS `nro_cuota`, `qt`.`idcuota` AS `idcuota`, `ct`.`idcontrato` AS `idcontrato`, `ct`.`n_expediente` AS `n_expediente`, `qt`.`monto_cuota` AS `monto_cuota`, sum(`detc`.`monto_pago`) AS `cancelado`, `qt`.`monto_cuota`- coalesce(sum(`detc`.`monto_pago`),0.00) AS `deuda`, `qt`.`fecha_vencimiento` AS `fecha_vencimiento`, (select `detalle_cuotas`.`fecha_pago` from `detalle_cuotas` where `detalle_cuotas`.`idcuota` = `qt`.`idcuota` order by `detalle_cuotas`.`iddetalle_cuota` desc limit 1) AS `fecha_pago`, `qt`.`estado` AS `estado`, `pers`.`nombres` AS `usuario`, `qt`.`inactive_at` AS `inactive_at` FROM ((((`cuotas` `qt` left join `detalle_cuotas` `detc` on(`detc`.`idcuota` = `qt`.`idcuota`)) join `contratos` `ct` on(`ct`.`idcontrato` = `qt`.`idcontrato`)) join `usuarios` `usu` on(`usu`.`idusuario` = `qt`.`idusuario`)) join `personas` `pers` on(`pers`.`idpersona` = `usu`.`idpersona`)) WHERE `detc`.`inactive_at` is null GROUP BY `qt`.`idcuota` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vws_list_refunds`  AS SELECT `dv`.`iddevolucion` AS `iddevolucion`, `dv`.`n_expediente` AS `n_expediente`, `ct`.`n_expediente` AS `expediente_contrato`, coalesce(`ct`.`n_expediente`,`sp`.`n_expediente`) AS `expediente_devuelto`, `dv`.`tipo_devolucion` AS `tipo_devolucion`, `dv`.`idcontrato` AS `idcontrato`, `dv`.`idseparacion` AS `idseparacion`, `ac`.`sublote` AS `sublote`, `py`.`denominacion` AS `denominacion`, coalesce(`ct`.`idcliente`,`lc`.`idcliente`) AS `idcliente`, coalesce(`ct`.`cliente`,`lc`.`cliente`) AS `cliente`, coalesce(`ct`.`documento_tipo`,`lc`.`documento_tipo`) AS `documento_tipo`, coalesce(`ct`.`documento_nro`,`lc`.`documento_nro`) AS `documento_nro`, coalesce(`ct`.`moneda_venta`,`sp`.`moneda_venta`) AS `moneda_venta`, `dv`.`monto_devolucion` AS `monto_devolucion`, `dv`.`create_at` AS `create_at`, `dv`.`imagen` AS `imagen`, `dv`.`detalle` AS `detalle`, `dv`.`porcentaje_penalidad` AS `porcentaje_penalidad`, `dv`.`modalidad_pago` AS `modalidad_pago`, `dv`.`entidad_bancaria` AS `entidad_bancaria`, `dv`.`nro_operacion` AS `nro_operacion`, `dv`.`inactive_at` AS `inactive_at` FROM ((((((`devoluciones` `dv` left join `vws_list_contracts` `ct` on(`ct`.`idcontrato` = `dv`.`idcontrato`)) left join `separaciones` `sp` on(`sp`.`idseparacion` = `dv`.`idseparacion`)) join `clientes` `cl` on(`dv`.`idcontrato` is null and `cl`.`idcliente` = `sp`.`idcliente` or `dv`.`idseparacion` is null and `cl`.`idcliente` = `ct`.`idcliente`)) join `vws_list_clients` `lc` on(`lc`.`idcliente` = `cl`.`idcliente`)) join `activos` `ac` on(`ac`.`idactivo` = coalesce(`ct`.`idactivo`,`sp`.`idactivo`))) join `proyectos` `py` on(`py`.`idproyecto` = `ac`.`idproyecto`)) ORDER BY `dv`.`iddevolucion` DESC ;
...
ALTER TABLE `contratos`
  ...
  ADD KEY `fk_idvendedor_cont` (`idvendedor`);
...
--
ALTER TABLE `configuraciones`
  MODIFY `idconfiguracion` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=12;
...
--
ALTER TABLE `detalles_contratos`
  MODIFY `iddetalle_contrato` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=6;

--
-- AUTO_INCREMENT de la tabla `detalle_costos`
--
ALTER TABLE `detalle_costos`
  MODIFY `iddetalle_costo` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=45;
...
--
ALTER TABLE `permisos`
  MODIFY `idpermiso` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=231;
...
--
ALTER TABLE `personas_juridicas`
  MODIFY `idpersona_juridica` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=7;
...
ALTER TABLE `contratos`
  ...
  ADD CONSTRAINT `fk_idvendedor_cont` FOREIGN KEY (`idvendedor`) REFERENCES `usuarios` (`idusuario`);
